...
static const char *alts[MAX_ALTS] =
{
...
void doReadall (void)
{
  ...
  else if ((model == PI_MODEL_CM1) || (model == PI_MODEL_CM3) || (model == PI_MODEL_CM3P) )  //could be Compute Module 1/3/3+ IO Board 120 GPIO pins, legacy code
  {
    allReadall ();
  }
  else
